...
function AsideMenu({
  ...
  return (
    <>
      {/* aside*/}
      <div
        className={`fixed top-0 [&_*]:!text-[#8D8D8D] hover:[&_*]:!text-[#43316E] ${lang == 'ar' ? 'right-0 rounded-bl-3xl rounded-tl-3xl' : 'left-0 rounded-br-3xl rounded-tr-3xl'} z-[100] flex h-svh w-[70%] min-w-[250px] max-w-[350px] items-start justify-center bg-white backdrop-blur-sm duration-500 sm:w-[calc(100%-100px)] md:w-[calc(100%-150px)] ${open ? 'translate-x-0' : lang == 'ar' ? 'translate-x-full' : '-translate-x-full'
          }`}
      >
        <div className='flex w-full flex-col items-start gap-1 px-4 text-base font-medium sm:px-6 sm:text-lg md:px-8 md:text-xl'>
          ...
          <ul className='flex w-full flex-col items-start gap-3 pb-4 sm:pb-6'>
            {menuItems?.map((item, i) => (
              <li
                key={item?.value}
                className={`${lang === 'ar' ? 'translate-x-[100%]' : 'translate-x-[-100%]'} ${open ? '!translate-x-0' : ''
